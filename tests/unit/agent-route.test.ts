...
function request(legalMoves:string[][]=[['card-a']]){
  return new Request('https://game.example/api/agent',{
    method:'POST',
    headers:{origin:'https://game.example','content-type':'application/json',cookie:providerCookie},
    body:JSON.stringify({observation:{seat:1,role:'boss',persona:'control',hand:[],level:2,turn:1,lastPlay:null,counts:[27,27,27,27],events:[],privateHands:['must-not-leave-route']},legalMoves}),
  });
}
...
describe('compatible agent route contract',()=>{
  ...
  it('cancels an oversized streamed provider response and releases capacity',async()=>{
    configure();vi.stubEnv('PAID_PROVIDER_MAX_INFLIGHT','1');
    const provider=vi.fn<typeof fetch>().mockResolvedValueOnce(new Response('x'.repeat(100_001))).mockResolvedValueOnce(Response.json({choices:[{message:{content:'{"move":["card-a"]}'}}]}));vi.stubGlobal('fetch',provider);
    expect((await POST(request())).status).toBe(502);expect((await POST(request())).status).toBe(200);expect(provider).toHaveBeenCalledTimes(2);
  });
  ...
  it('aborts a slow provider and exposes only the timeout contract',async()=>{
    ...
    vi.stubGlobal('fetch',provider);
    const pending=POST(request());await vi.waitFor(()=>expect(provider).toHaveBeenCalledOnce());
  ...
  it('requires a live signed session and enforces the durable daily budget',async()=>{
    configure();vi.stubEnv('PAID_PROVIDER_USER_DAILY_UNITS','1');vi.stubEnv('PAID_PROVIDER_GLOBAL_DAILY_UNITS','1');
    const provider=vi.fn<typeof fetch>(async()=>Response.json({choices:[{message:{content:'{"move":["card-a"]}'}}]}));vi.stubGlobal('fetch',provider);
    const unsigned=new Request('https://game.example/api/agent',{method:'POST',headers:{origin:'https://game.example','content-type':'application/json'},body:JSON.stringify({observation:{seat:1,role:'boss',persona:'control',hand:[],level:2,turn:1,lastPlay:null,counts:[27,27,27,27],events:[]},legalMoves:[['card-a']]})});
    expect((await POST(unsigned)).status).toBe(401);
    expect((await POST(request())).status).toBe(200);
    const limited=await POST(request());expect(limited.status).toBe(429);expect(limited.headers.get('retry-after')).toBeTruthy();expect(provider).toHaveBeenCalledOnce();
  });

  it('fails closed when explicit paid-provider budgets are missing',async()=>{
    configure();vi.stubEnv('PAID_PROVIDER_USER_DAILY_UNITS','');vi.stubEnv('PAID_PROVIDER_GLOBAL_DAILY_UNITS','');
    const provider=vi.fn<typeof fetch>();vi.stubGlobal('fetch',provider);
    const response=await POST(request());expect(response.status).toBe(503);expect(provider).not.toHaveBeenCalled();
  });

  it('rejects malformed nested observations before leasing capacity or charging budget',async()=>{
    configure();vi.stubEnv('PAID_PROVIDER_MAX_INFLIGHT','1');const provider=vi.fn<typeof fetch>(async()=>Response.json({choices:[{message:{content:'{"move":["card-a"]}'}}]}));vi.stubGlobal('fetch',provider);
    const malformed=new Request('https://game.example/api/agent',{method:'POST',headers:{origin:'https://game.example','content-type':'application/json',cookie:providerCookie},body:JSON.stringify({observation:{seat:1,role:'boss',persona:'control',hand:[null],level:2,turn:1,lastPlay:null,counts:[27,27,27,27],events:[]},legalMoves:[['card-a']]})});
    expect((await POST(malformed)).status).toBe(400);expect((await POST(request())).status).toBe(200);expect(provider).toHaveBeenCalledOnce();
  ...
  it('opens the shared AI circuit after consecutive upstream failures',async()=>{
    configure();vi.stubEnv('PROVIDER_CIRCUIT_FAILURE_THRESHOLD','2');vi.stubEnv('PROVIDER_CIRCUIT_OPEN_SECONDS','30');
    const provider=vi.fn<typeof fetch>(async()=>Response.json({error:'unavailable'},{status:503}));vi.stubGlobal('fetch',provider);
    expect((await POST(request())).status).toBe(502);expect((await POST(request())).status).toBe(502);
    const open=await POST(request());expect(open.status).toBe(503);expect(open.headers.get('retry-after')).toBe('30');expect(provider).toHaveBeenCalledTimes(2);
  });
});
