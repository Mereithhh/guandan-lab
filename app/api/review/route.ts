...
export async function POST(request:Request){
  ...
  if(!consumeRateLimit(requestClientKey(request,'review',process.env.TRUST_PROXY==='1'),10,60_000))return Response.json({error:'复盘请求过于频繁'},{status:429});
  ...
  try{
    ...
    if(!response.ok)return Response.json({error:'远程 AI 暂时不可用'},{status:502});const raw=await response.text();if(raw.length>120_000)return Response.json({error:'远程 AI 响应过大'},{status:502});
    const data=JSON.parse(raw) as {choices?:Array<{message?:{content?:string}}>},content=data.choices?.[0]?.message?.content;if(typeof content!=='string'||!content.trim())return Response.json({error:'远程 AI 返回为空'},{status:502});
    const review=parseRemoteMatchReview(content);if(!review)return Response.json({error:'远程 AI 复盘格式无效'},{status:502});return Response.json({review,provider:'compatible'});
  }catch{return Response.json({error:controller.signal.aborted?'远程 AI 请求超时':'远程 AI 响应无效'},{status:controller.signal.aborted?504:502})}finally{clearTimeout(timer)}
}
