...
export const ENDGAME_PUZZLES:EndgamePuzzle[]=[
  ...
  {id:'protect-partner-ace',seed:2026082204,title:text('搭档报单，别主动盖 A','Partner has one card — do not cover the ace'),scene:text('小顾只剩 1 张，刚出 A，林姐已过牌；现在轮到你，王总尚未行动。','Gu has one card left and played an ace. Lin passed; it is your turn, and Wang has not acted yet.'),prompt:text('哪手既不盖住搭档当前的 A，又保留你的级牌和大王？','Which choice avoids covering your partner’s ace and keeps both your level card and big joker?'),level:7,leader:2,remaining:[6,2,1,3],publicPasses:1,learnerHand:cards('0-H-7','0-J-16','0-S-4','0-H-4','0-C-9','0-D-9'),lastPlay:last(2,7,'0-S-14'),options:[pass('符合目标：不盖住 A，同时保留级牌和大王。','Matches the goal: do not cover the ace, and keep both the level card and big joker.'),play(['0-H-7'],'规则合法，但会盖住搭档的 A 并用掉级牌。','Rules-legal, but it covers your partner’s ace and spends the level card.'),play(['0-J-16'],'规则合法，但会盖住搭档的 A 并用掉大王。','Rules-legal, but it covers your partner’s ace and spends the big joker.')],best:0,rule:text('级牌的单张点数高于 A、低于小王和大王。本题按明确的“让牌权并保留两张硬牌”目标评分，不预测王总是否能压。','The level card ranks above ace and below the jokers. This puzzle scores the explicit goal of yielding and keeping both strong cards, without predicting Wang’s response.'),social:text('不主动抢搭档牌权是可讨论的合作建议，不是暗号，也不是对手一定过牌的保证。','Not stealing control is a discussable teamwork recommendation, not a signal or a guarantee that the opponent will pass.')},
  {id:'level-over-ace',seed:2026082205,title:text('级牌压 A，王留后手','Use the level card; keep the joker'),scene:text('当前打 7。王总出单张 A，小顾和林姐都已过牌，现在轮到你。','Sevens are level. Wang played a single ace; Gu and Lin passed, and it is your turn.'),prompt:text('如果要拿牌权，哪张是刚好够大的单牌？','If you want control, which single is just large enough?'),level:7,leader:1,remaining:[6,3,2,2],publicPasses:2,learnerHand:cards('0-S-7','0-J-16','0-S-4','0-H-4','0-C-9','0-D-9'),lastPlay:last(1,7,'1-S-14'),options:[play(['0-S-7'],'正确：任意花色的级牌作单张都大于 A，已经足够。','Correct: any level seven played as a single beats an ace, so this is enough.'),play(['0-J-16'],'合法但过度：大王能赢，却不是“刚好够大”。','Legal, but excessive: the big joker wins and is not the smallest winning single.'),pass('合法，但没有完成题面“拿到牌权”的目标。','Legal, but it does not achieve the stated goal of taking control.')],best:0,rule:text('当前级牌作单张时高于 A；只有小王、大王更大。红桃级牌的“逢人配”只在多张组合里生效。','The current level card beats an ace as a single; only the jokers are higher. The heart level card is wild only inside multi-card combinations.'),social:text('用刚好够大的牌回应王总，既认真竞技，也给后续配合留余地。','Using exactly enough strength against Wang shows honest competition and preserves options for later teamwork.')},
];
...
export function puzzleSummary(puzzle:EndgamePuzzle,locale:PuzzleLocale){
  const names=locale==='zh'?['你','王总','小顾','林姐']:['You','Wang','Gu','Lin'];
